...
def mpjve(predicted, target, axis=0):
    """
...
def p_mpjpe(predicted, target):
    ...
    """
    assert predicted.shape == target.shape    #t,j,3
...
def mean_velocity_error_torch(predicted, target, axis=0):
    """
    Mean per-joint velocity error (i.e. mean Euclidean distance of the 1st derivative)
    """
    assert predicted.shape == target.shape
...
class AccumLoss(object):
    ...
    def update(self, val, n=1):
        ...
        self.sum += val
        self.count += n
        self.avg = self.sum * 1000. / self.count



def save_ckpt(state, ckpt_path, file_name='ckpt_last.pth.tar'):
