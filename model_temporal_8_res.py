...
class Encoder(nn.Module):
    def __init__(self, layers, d_model_ls):
...
class Transformer(nn.Module):
    ...
    def make_model_enco(self, d_model=256, d_ff=512, h=8, dropout=0.1):
        ...
        attn4 = Sparse_MHAT(h, d_model, M=3, dropout=dropout)
        attn5 = Sparse_MHAT(h, d_model, M=3, dropout=dropout)
        attn6 = Sparse_MHAT(h, d_model, M=3, dropout=dropout)
        attn7 = Sparse_MHAT(h, d_model, M=3, dropout=dropout)
        attn8 = Sparse_MHAT(h, d_model, M=3, dropout=dropout)
        ...
        enco_layers = nn.ModuleList([EncoderLayer(attn1, c(ff), d_model, d_model, dropout),
                                     ...
                                     EncoderLayer(attn4, c(ff), d_model, d_model, dropout),
                                     EncoderLayer(attn5, c(ff), d_model, d_model, dropout),
                                     EncoderLayer(attn6, c(ff), d_model, d_model, dropout),
                                     EncoderLayer(attn7, c(ff), d_model, d_model, dropout),
                                     EncoderLayer(attn8, c(ff), d_model, d_model, dropout)])
        # model = Encoder(EncoderLayer(c(attn), c(ff), d_model, d_model, dropout), N)
        model = Encoder(enco_layers, embed_dims)
        return model
