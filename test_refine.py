...
from common.generators_stride_rooling_1 import UnchunkedGenerator
import numpy as np
from model_temporal_8_res import Transformer
from model_refining import Refine_Model
...
def model_eval(input_2D, batch_cam, gt_3D_single, model_trans, model_refine, joints_embed):
    output_3D, output_3D_enco = model_trans(input_2D+joints_embed)
    output_3D_single = output_3D
    uvd = torch.cat((input_2D[:, pad, :, :].unsqueeze(1), output_3D_single[:, :, :, 2].unsqueeze(-1)), -1)
    xyz = get_uvd2xyz(uvd, gt_3D_single, batch_cam)
    xyz[:, :, 0, :] = 0
    post_out = model_refine(output_3D_single, xyz)
    return post_out

def input_augmentation(input_2D, input_3D, input_cam, model_refine, model_trans, joints_left, joints_right, joints_embed):

    input_2D_aug = copy.deepcopy(input_2D)
    input_2D_aug[:, :, :, 0] *= -1
    input_2D_aug[:, :, joints_left + joints_right] = input_2D_aug[:,:, joints_right + joints_left]

    input_3D_aug = copy.deepcopy(input_3D)
    input_3D_aug[:, :, :, 0] *= -1
    input_3D_aug[:, :, joints_left + joints_right] = input_3D_aug[:, :, joints_right + joints_left]

    batch_cam_aug = copy.deepcopy(input_cam)
    batch_cam_aug[:, 2] *= -1
    batch_cam_aug[:, 7] *= -1

    post_out = model_eval(input_2D, input_cam, input_3D, model_trans, model_refine, joints_embed)
    post_out_aug = model_eval(input_2D_aug, batch_cam_aug, input_3D_aug, model_trans, model_refine, joints_embed)


    post_out_aug[:,:,:, 0] *= -1
    post_out_aug[:, :, joints_left + joints_right] = post_out_aug[:,:, joints_right + joints_left]
    output_3D = (post_out_aug + post_out) / 2
    return output_3D
...
if __name__ == '__main__':
    ...
    model_refine.load_state_dict(ckpt['state_dict_refine'])
    ...
    with torch.no_grad():
        ...
        for action_key in all_actions.keys():
            ...
            out_poses_3d, out_poses_2d, out_cams = fetch_actions(infos)

            test_generator = UnchunkedGenerator(batch_size=args.batch_size_test, cameras=out_cams, poses_3d=out_poses_3d,
            ...
            for batch_test, (test_cam, test_3d, test_2d) in enumerate(test_generator.next_epoch()):
                test_3d = torch.from_numpy(test_3d.astype('float32')).cuda()
                test_2d = torch.from_numpy(test_2d.astype('float32')).cuda()
                test_cam = torch.from_numpy(test_cam.astype('float32')).cuda()


                N_test = test_3d.shape[0]
                if test_time_augmentation:
                    output_3D_test = input_augmentation(test_2d, test_3d, test_cam, model_refine, model, joints_left, joints_right, joints_embed)
